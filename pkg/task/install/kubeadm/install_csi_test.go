...
func TestCsi_Name(t1 *testing.T) {
	tests := []struct {
		name string
		want string
	}{
		{name: "install-csi", want: "install-csi"},
	}
...
func TestNewInstallCSI(t *testing.T) {
	tests := []struct {
		name string
		want *Csi
	}{
		{name: "NewInstallCSI", want: NewInstallCSI()},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewInstallCSI(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewInstallCSI() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestCsi_Run(t1 *testing.T) {
	type args struct {
		ctx         context.Context
	...
	tests := []struct {
		...
		{name: "kubeadm-install-cluster-csi", args: struct {
			ctx         context.Context
			status      cluster.Status
			clusterSpec *v1alpha1.ClusterSpec
			ou          osutility.OSUtil
